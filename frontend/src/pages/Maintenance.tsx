    ...
    try {
      ...
      const mapped: MaintRow[] = logsArray.map((m: MaintenanceLog) => {
        ...
        return {
          ...
          status: m.status,
          status_before: m.status_before || 'working',
          ...
          <DialogContent>
            <Stack spacing={2}>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                ...
                <TextField select label="Current Status" value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value as any })} fullWidth>
                  {STATUS.map((s) => (
                    <MenuItem key={s} value={s}>{s}</MenuItem>
                  ))}
                </TextField>
                {formData.status === 'fixed' && (
                  <TextField select label="Status After Fix" value={formData.status_after} onChange={(e) => setFormData({ ...formData, status_after: e.target.value as any })} fullWidth>
                    {EQUIPMENT_STATUS.map((s) => (
                      <MenuItem key={s} value={s}>{s.replace('_', ' ')}</MenuItem>
                    ))}
                  </TextField>
